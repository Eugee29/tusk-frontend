   ...
   return (
      <div className='group-preview archived'>
         <div className="task-list archived">
            {tasks.map((task, index) => task.task.archivedAt && <ArchivedPreview key={task.task.id} board={board} groupId={task.groupId} task={task.task} index={index} onUpdateBoard={onUpdateBoard} />)}
         </div>
      </div>
   )
}
